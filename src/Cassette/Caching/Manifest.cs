using System;
using System.Collections.Generic;

namespace Cassette.Caching
{
    class Manifest
    {
        public static Manifest Precompiled(IEnumerable<Bundle> bundles)
        {
            return new Manifest(bundles, "")
            {
                IsPrecompiled = true
            };
        }

        public IEnumerable<Bundle> Bundles { get; private set; }
        public DateTime CreationDateTime { get; private set; }
        public string Version { get; private set; }
        public bool IsPrecompiled { get; private set; }

        public Manifest(IEnumerable<Bundle> bundles, string version, DateTime creationDateTime, bool isPrecompiled)
        {
            Bundles = bundles;
            Version = version;
            CreationDateTime = creationDateTime;
            IsPrecompiled = isPrecompiled;
        }

        public Manifest(IEnumerable<Bundle> bundles, string version)
        {
            Bundles = bundles;
            Version = version;
            CreationDateTime = DateTime.UtcNow;
        }
    }
}